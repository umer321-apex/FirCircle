  ...
  useEffect(() => {
    ...
    const setup = async () => {
      ...
      socket.on('newMessage', (msg) => {
        ...
        if (isRelevant && !msg.podId) setMessages((prev) => [...prev, msg]);
      });
...
>
      ...
      {isTyping && <Text style={styles.typingIndicator}>{otherUserName} is typing…</Text>}
...
const createStyles = (theme) =>
  StyleSheet.create({
    ...
    bubble: { paddingHorizontal: theme.spacing.md, paddingVertical: theme.spacing.sm + 2, borderRadius: theme.radius.xl },
